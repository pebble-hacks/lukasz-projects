...
char day_text[] = "23RD";
char dad_text[] = "ILUVUDAD";
char grid[16][2];
bool msg_on = false;
AppTimer *timer_switch;
...
static void timer_switch_callback(void *context) {
  if(msg_on==true)
  {
    hide_message();
  }
...
static void handle_accl_tap(AccelAxisType axis, int32_t direction){
  if(msg_on==false){
    show_message();
    timer_switch = app_timer_register(1500, timer_switch_callback, NULL);
  }
}
...
static void init() {
  ...
  accel_tap_service_subscribe(handle_accl_tap);
}
